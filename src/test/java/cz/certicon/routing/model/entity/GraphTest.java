...
public class GraphTest {
    ...
    static final SimpleDistanceFactory distanceFactory = new SimpleDistanceFactory();
    private final GraphEntityFactory graphFactory;

    public GraphTest( GraphEntityFactory graphFactory ) {
    ...
    public void testAddNode() {
        ...
        Node node = graphFactory.createNode( LOOPS + 1, LOOPS + 1 );
    ...
    public void testAddEdge() {
        ...
        Graph instance = initGraph( graphFactory, LOOPS );
        Node a = graphFactory.createNode( LOOPS - 1, 0 );
        Node b = graphFactory.createNode( 0, LOOPS - 1 );
    ...
    public void testGetNodes() {
        ...
        for ( int i = 0; i < LOOPS; i++ ) {
            for ( int j = 0; j < LOOPS; j++ ) {
                expResult.add( graphFactory.createNode( i, j ) );
            }
        }
    ...
    private static Graph initGraph( GraphEntityFactory factory, int loops ) {
        ...
        for ( int i = 0; i < loops; i++ ) {
            for ( int j = 0; j < loops; j++ ) {
                instance.addNode( factory.createNode( i, j ) );
            }
        }
        return instance;
    }
    ...
    private static Graph initGraphWithEdges( GraphEntityFactory factory, int loops ) {
        ...
        for ( Node a : instance.getNodes() ) {
            for ( Node b : instance.getNodes() ) {
                if ( !a.equals( b ) ) {
                    instance.addEdge( createEdge( factory, distanceFactory, a, b ) );
                }
            }
        }
        return instance;
