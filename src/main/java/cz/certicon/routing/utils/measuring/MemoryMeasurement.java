...
public class MemoryMeasurement {
    ...
    public long stop() {
        ...
        memory = Runtime.getRuntime().totalMemory() - start;
     ...
     * only works on the HotSpot JVM, and infers the actual memory layout (32
     * bit vs. 64 bit word size, compressed object pointers vs. uncompressed)
     * from best available indicators. It can reliably detect a 32 bit vs. 64
     * bit JVM. It can only make an educated guess at whether compressed OOPs
     * are used, though; specifically, it knows what the JVM's default choice of
     * OOP compression would be based on HotSpot version and maximum heap sizes,
     * but if the choice is explicitly overridden with the
     * <tt>-XX:{+|-}UseCompressedOops</tt> command line switch, it can not
     * detect this fact and will report incorrect sizes, as it will presume the
    ...
    public static class ObjectSizeCalculator {
        ...
        public interface MemoryLayoutSpecification {
            ...
            /**
             * Returns the fixed overhead of an array of any type or length in
             * this JVM.
             *
            ...
            /**
             * Returns the fixed overhead of for any {@link Object} subclass in
             * this JVM.
             *
            ...
            /**
             * Returns the quantum field size for a field owned by an object in
             * this JVM.
             *
            ...
            /**
             * Returns the quantum field size for a field owned by one of an
             * object's ancestor superclasses in this JVM.
             *
         ...
         * object and all other objects reachable from it. Attempts to to detect
         * the current JVM memory layout, but may fail with
         * {@link UnsupportedOperationException};
         *
         * @param obj the object; can be null. Passing in a
         * {@link java.lang.Class} object doesn't do anything special, it
         * measures the size of all objects reachable through it (which will
         * include its class loader, and by extension, all other Class objects
         * loaded by the same loader, and all the parent class loaders). It
         * doesn't provide the size of the static fields in the JVM class that
         * the Class object represents.
         * @return the total allocated size of the object and all other objects
         * it retains.
        ...
        /**
         * Creates an object size calculator that can calculate object sizes for
         * a given {@code memoryLayoutSpecification}.
         *
         * @param memoryLayoutSpecification a description of the JVM memory
         * layout.
         */
