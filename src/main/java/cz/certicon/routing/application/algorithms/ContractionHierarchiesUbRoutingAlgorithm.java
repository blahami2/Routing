 ...
 * {@link cz.certicon.routing.application.preprocessing.ch.ContractionHierarchiesPreprocessor}.
 ...
 * where they meet. Every time they meet, an upper bound is created based on the
 * minimum path length, limiting the search space.
 *
 * @author Michael Blaha {@literal <michael.blaha@certicon.cz>}
 */
public class ContractionHierarchiesUbRoutingAlgorithm extends AbstractRoutingAlgorithm {
    ...
    /**
     * Constructor for {@link ContractionHierarchiesUbRoutingAlgorithm}
     *
    ...
    public <R> R route( RouteBuilder<R, Graph> routeBuilder, NodeSet<Graph> nodeSet, Map<Integer, NodeSet.NodeEntry> from, Map<Integer, NodeSet.NodeEntry> to, float upperBound ) throws RouteNotFoundException {
        ...
        initArrays( graph, nodeFromDistanceArray, nodeFromDataStructure, from );
        // add target points for the "to" Dijkstra
        initArrays( graph, nodeToDistanceArray, nodeToDataStructure, to );
        int finalNode = -1;
...
//                System.out.println( "F: distance = " + currentDistance );
                ...
                if ( finalDistance < currentDistance ) {
                    ...
                    // check if the node is a meeting node, update upper bound if necessary
